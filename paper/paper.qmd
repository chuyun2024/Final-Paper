...
ggplot(casualties_by_type, aes(x = reorder(shooting_type, total_casualties), y = total_casualties, fill = shooting_type)) +
  ...
  labs(
    title = "Distribution of Casualty By Shooting Type",
    x = "Shooting Type",
...
ggplot(map_data, aes(long, lat, group = group, fill = total_casualties)) +
  ...
  labs(
    title = "US School Shooting Casualties Visualization",
    x = NULL,
    y = NULL
  ) +
  coord_fixed(1.3) +
...
ggplot(data, aes(x = lat)) +
  ...
  theme_minimal() +
  labs(
    title = "Distribution of Latitude of Schools in Shooting Incidents",
    x = "Latitude",
    y = "Frequency"
  ) +
  theme(
...
state_data <- state_data %>%
  mutate(
    exist_targeted = ifelse(prob_targeted > 0.70657, 1, 0),
    exist_other = ifelse(prob_other > 0.70657, 1, 0)
  )
...
The Washington Post’s investigation into school shootings exemplifies a meticulous approach to data collection and analysis, aligning with best practices in data journalism and social science research.
